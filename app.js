/* [JS Version: v1.2.0] 最終更新: タブレット縦画面（16:10）ベース表示の一本化・セーフエリア対応（完全安定版） */
...
/* ----------------------------------------------------
 * 3. 新・戦績＆データ管理 (LocalStorage)
 * ---------------------------------------------------- */
const StorageManager = {
  VERSION_KEY: 'royalStatsSchemaVersion',
  CURRENT_SCHEMA_VER: 'v1.1.0',
...
function renderRankingModalContent() {
  ...
  if (currentStatsTab === 'all') {
    ...
    let html = `
      <div style="margin-bottom: 8px; font-size: 12px; color: #b0bec5;">
        宮廷総合格付け（全11名・大富豪率 順）<br>
        <span style="font-size:10px; color:#8c9ba5;">※AIキャラは自動操作時のみ、あなたは手動操作時のみ集計されます。</span>
      </div>
    ...
    list.forEach((c, idx) => {
      ...
      const avatarEl = c.isPlayer
        ? `<div style="width: 22px; aspect-ratio: 2/3; border-radius: 3px; border: 1px solid rgba(212,175,55,0.8); background: #1a2332; display: flex; align-items: center; justify-content: center; font-size: 13px;">👤</div>`
    ...
    for (let c in stats.cpuStats) {
      if (stats.cpuStats[c].beatMe > maxBeat) {
        ...
        nemesis = CHARACTER_DEFS[c] ? `${CHARACTER_DEFS[c].icon} ${CHARACTER_DEFS[c].name}` : c;
      }
    }
    ...
    body.innerHTML = `
      ...
      <div style="margin-bottom: 14px;">
        <div class="rule-item-title">🔄 上納・搾取・下克上記録</div>
        <div class="stats-grid">
      ...
      <div>
        <div class="rule-item-title">⚔️ あなたの天敵AI</div>
        <div style="background: rgba(0,0,0,0.3); padding: 8px 12px; border-radius: 6px;">
          <div>最も敗北を喫した相手: <strong style="color:#ff6b6b; font-size: 14px;">${nemesis}</strong></div>
        </div>
      </div>
    `;
  }
